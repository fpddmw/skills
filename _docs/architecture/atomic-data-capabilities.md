...
AirNow 与 Federal Register 是首批落地实例，USGS Water IV 与 Open-Meteo Air Quality
是前两个逐项后续迁移实例。本地候选分支可先完成薄化和测试，但只有在对应 CLI 正式
版本发布、binding 重生成、隔离安装 smoke 和回退路径验证后才达到生产完成。其他候选
继续逐个执行同一门槛。
