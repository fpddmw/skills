#!/bin/bash
# SCI-source search wrapper over Tiangong AI CLI.
...
case "$SOURCE_INPUT" in
    ""|"default"|"sci")
        ;;
    *)
        echo "Error: tiangong-kb-sci-search searches only the sci source; use the report or patent skill for other sources" >&2
        exit 2
        ;;
esac
